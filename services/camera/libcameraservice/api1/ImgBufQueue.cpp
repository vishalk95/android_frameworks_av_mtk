...
*
*******************************************************************************/
#define MY_LOGV(fmt, arg...)        ALOGV("(%s)[%s] " fmt, getQueName(), __FUNCTION__, ##arg)
#define MY_LOGD(fmt, arg...)        ALOGD("(%s)[%s] " fmt, getQueName(), __FUNCTION__, ##arg)
#define MY_LOGI(fmt, arg...)        ALOGI("(%s)[%s] " fmt, getQueName(), __FUNCTION__, ##arg)
#define MY_LOGW(fmt, arg...)        ALOGW("(%s)[%s] " fmt, getQueName(), __FUNCTION__, ##arg)
#define MY_LOGE(fmt, arg...)        ALOGE("(%s)[%s] " fmt, getQueName(), __FUNCTION__, ##arg)
#define MY_LOGA(fmt, arg...)        ALOGA("(%s)[%s] " fmt, getQueName(), __FUNCTION__, ##arg)
#define MY_LOGF(fmt, arg...)        ALOGF("(%s)[%s] " fmt, getQueName(), __FUNCTION__, ##arg)
//
...
enqueProvider(ImgBufQueNode const& rNode)
{
    ...
    MY_LOGV_IF(
        ...
        (int) mDoneImgBufQue.size(), 
        rNode.getCookieED(), rNode.getCookieDE(), ImgBufQueNodeStatus2Name(rNode.getStatus()), 
        rNode->getBufName(), rNode->getVirAddr(), (int) rNode->getBufSize(), rNode->getImgFormat().string(), 
...
stopProcessor()
{
    ...
    if  ( ! mDoneImgBufQue.empty() ) {
        MY_LOGW("intent to clear Done Que: size(%d)!=0", (int) mDoneImgBufQue.size());
...
flushProcessor()
{
    ...
    //
    MY_LOGD("TODO Que.size(%d)", (int) mTodoImgBufQue.size());
    for ( List<ImgBufQueNode>::iterator it = mTodoImgBufQue.begin(); it != mTodoImgBufQue.end(); it++ )
    {
        ...
        MY_LOGD_IF(
            ...
            (*it)->getBufName(), (*it)->getVirAddr(), (int) (*it)->getBufSize(), (*it)->getImgFormat().string(), 
...
enqueProcessor(ImgBufQueNode const& rNode)
{
    ...
    MY_LOGV_IF(
        ...
        (int) mTodoImgBufQue.size(), ImgBufQueNodeStatus2Name(rNode.getStatus()), rNode.getCookieED(), rNode.getCookieDE(), 
        rNode->getBufName(), rNode->getVirAddr(), (int) rNode->getBufSize(), rNode->getImgFormat().string(), 
...
dequeProcessor(Vector<ImgBufQueNode>& rvNode)
{
    ...
    while   ( mDoneImgBufQue.empty() && mbIsProcessorRunning )
    {
        ...
        if  ( NO_ERROR != status )
        {
            MY_LOGW("wait status(%d), Que.size(%d), IsProcessorRunning(%d)", status, (int) mDoneImgBufQue.size(), mbIsProcessorRunning);
        }
    }
    //
    if  ( ! mDoneImgBufQue.empty() )
