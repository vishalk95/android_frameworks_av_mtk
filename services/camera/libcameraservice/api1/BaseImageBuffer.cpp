...
#define MY_LOGA(fmt, arg...)        ALOGA("(%s)[%s] " fmt, getMagicName(), __FUNCTION__, ##arg)
...
onCreate()
{
    ...
    {   // create non-BLOB image buffer from BLOB heap.
		MY_LOGE("format is BLOB %d IMG %d",mspImgBufHeap->getImgFormat(),getImgFormat());
...
getBufVA(size_t index) const
{
    ...
    {
        MY_LOGW("This call is legal only after lockBuf() with SW usage - LockCount:%d Usage:%#x", mLockCount, mLockUsage);
        dumpCallStack(__FUNCTION__);
        return  0;
    }
    //
