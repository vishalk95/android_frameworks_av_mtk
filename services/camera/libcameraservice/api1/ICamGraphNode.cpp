        ...
        ICamNodeImpl* const pNode_impl)
{
    ...
    if( pNode_impl )
        MY_LOGV("connect data(%d), dstdata(%d), node(0x%x)", srcdata, dstdata, (int)(size_t) pNode_impl->mpSelf);
        ...
        ICamNodeImpl* const pNode_impl)
{
    ...
    if( pNode_impl )
        MY_LOGV("connect msg(0x%x), node(0x%x)", msg, (int)(size_t) pNode_impl->mpSelf);
...
handlePostBuffer(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext) const
{
    ...
    MY_LOGV("post data(%d), buf(0x%x), ext(0x%x) -> data(%d) %s(0x%x)",
            data, (int)(size_t) buf, ext, 
            targetData, nodeImpl->getName(), (int)(size_t) nodeImpl->mpSelf);
            ...
            if( HAVE_NOTIFY(*iter_msg, msg) )
            {
                ...
                if( !ret )
                {
                    MY_LOGE("notify error at msg(0x%x), node(%s/0x%x)",
                            msg, nodeImpl->getName(), (int)(size_t) nodeImpl->mpSelf);
                }
                break;
            }
        }
...
dumpDataLink() const
{
    ...
    for( iter = mlDataLink.begin(); iter != mlDataLink.end(); iter++ )
    {
        MY_LOGW("data(%d->%d), %s(0x%x)",
                iter->srcdata, iter->dstdata, iter->nodeImpl->getName(), (int)(size_t) iter->nodeImpl->mpSelf );
    }
}
...
setBufferHandler(MUINT32 const data, ICamBufHandler * const pBufHdl)
{
    ...
    for( iter = mvBufHdl.begin() ; iter != mvBufHdl.end() ; iter++ )
    {
        ...
        for( data_iter = iter->mvData.begin(); data_iter != iter->mvData.end(); data_iter++ )
        {
            if( *data_iter == data )
            {
                MY_LOGV("(0x%x) buffer handler already added: %d/0x%x", (int)(size_t) this, *data_iter, (int)(size_t) iter->pHandle);
                return MTRUE;
            }
        }
        // if same handler
        if( iter->pHandle == pBufHdl )
        {
            ...
            MY_LOGD("(0x%x) update to same hdl: data(%d) hdl(0x%x)", (int)(size_t) this, data, (int)(size_t) iter->pHandle);
            return MTRUE;
        }
    }

    MY_LOGD("(0x%x) new bufhdl data(%d), hdl(0x%x)", (int)(size_t) this, data, (int)(size_t) pBufHdl);
    BufHdlMap newhdl;
        ...
        )
{
    ...
    if( !(pUpNode && pDownNode) ) {
        MY_LOGE("up(0x%x), down(0x%x)", (int)(size_t) pUpNode, (int)(size_t) pDownNode );
        goto lbExit;
    }
    MY_LOGD("data(%d->%d), %s/%s (%d:0x%x/%d:0x%x)",
            updata, downdata,
            pUpNode->getName(), pDownNode->getName(),
            pUpNode->getSensorIdx(), (int)(size_t) pUpNode, 
            pDownNode->getSensorIdx(), (int)(size_t) pDownNode);
    {
        ICamNodeImpl* up_impl   = pUpNode->mpImpl;
...
#define ASSERT_STAGE( eCmd, exp, eNextstage )                                   \
        ...
        default:
            MY_LOGE("should not happen cmd(0x%x)", (int)(size_t) cmd);
            mStage = ThreadState_Idle;
            return MFALSE;
            break;
#undef ASSERT_STAGE
